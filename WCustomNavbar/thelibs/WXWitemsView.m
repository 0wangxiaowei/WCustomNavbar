...
-(id)init
{
    ...
    if(self){
        ...
        _linePercent = 0.8;
        _lineHieght = 2.5;//线条的高度
    }
    return self;
}

@end
@interface WXWitemsView()

@property(nonatomic,strong)UIView *line;


@end
@implementation WXWitemsView
...
-(void)setTitleArray:(NSArray *)titleArray
{
    ...
    for (int i=0; i<titleArray.count; i++) {
        ...
        if(i==0){
            ...
            _currentIndex = 0;
            self.line = [[UIView alloc] initWithFrame:CGRectMake(_config.itemWidth*(1-_config.linePercent)/2.0, CGRectGetHeight(self.frame) - _config.lineHieght-20, _config.itemWidth*_config.linePercent, _config.lineHieght)];
...
-(void)itemButtonClicked:(UIButton*)btn
{
    ...
    if([_delegate1 respondsToSelector:@selector(setTapItemWithIndex:)])
    {
        //send the delegate function with the amount entered by the user
        [_delegate1 setTapItemWithIndex:_currentIndex];
    }
